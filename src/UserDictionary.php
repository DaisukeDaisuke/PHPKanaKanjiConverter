...
{
	public const MODE_KANA       = 0;
	public const MODE_NO_CONVERT = 1;
	public const MODE_REPLACE    = 2;
	public const MODE_KANA_ALT   = 3;
	...
	public function add(array $entry): void
	{
		...
		// MODE_KANA_ALT: ローマ字キー（省略時は reading をそのままキーとして使う）
		if ($mode === self::MODE_KANA_ALT) {
			$normalized['romaji'] = $this->sanitize((string)($entry['romaji'] ?? $reading));
		}
	...
	/**
	 * reading で完全一致するエントリを返す
	 *
	 * @return list<array<string,mixed>>
	 */
	public function findByReading(string $reading): array
	{
		return array_values(
			array_filter($this->entries, static fn(array $e): bool => $e['reading'] === $reading)
		);
	}

	/**
	 * MODE_KANA_ALT のかな変換テーブルを返す
	 * ConvertibleRomaji::addCustomRule() に渡す形式: ['romaji' => 'かな出力']
	 *
	 * @return array<string,string>  キー: ローマ字、値: かな
	 */
	public function getKanaAltRules(): array
	{
		$rules = [];
		foreach ($this->getByMode(self::MODE_KANA_ALT) as $entry) {
			$romaji = $entry['romaji'] ?? $entry['reading'];
			// 同じキーが複数ある場合は先勝ち（addAll の順序が優先）
			if (!isset($rules[$romaji])) {
				$rules[$romaji] = $entry['reading']; // reading = かな出力
			}
		}
		return $rules;
	}
	...
	/**
	 * MODE_KANA のエントリを KanaKanjiConverter 形式に変換して返す
	 * （ローマ字→かな と かな→漢字の両フェーズで使われる）
	 *
	 * @return array<string, list<array<string,mixed>>>  読み => [エントリ配列]
	 */
	public function getKanaModeEntries(): array
	{
		$result = [];
		foreach ($this->getByMode(self::MODE_KANA) as $entry) {
